...
export const ModalNovoLivro = ({
  ...
  const [dadosLivro, setDadosLivro] = useState<IDadosCadastroLivro>({
    titulo: "",
    autor: "",
    editora: "",
    anoPublicacao: 0,
  ...
  const editarLivro = async () => {
    setLoading(true);
    try {
      await api.post(`/Livro/Cadastrar`, dadosLivro);
      toast({
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      <ModalOverlay />
      <ModalContent>
        <ModalHeader>Adicionar novo livro</ModalHeader>
        <ModalCloseButton />
        <ModalBody>
